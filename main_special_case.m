...
num_frame = 500; %num_frame frames are used
...
Aoi_app(iave) =    N*T*2*M*exp(t_opt)/t_opt/(exp(t_opt/2)+1+t_opt/2);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
J=0; % record the number of successful updates
x_j = 0; 
p_tx =  0.1* ones(1,M);% 1./(M-[0:M-1]);%0.1* ones(1,M);%
p_tx =   1./(M-[0:M-1]);%
...
ratiox(iave) = 2*exp(t_opt-1)/t_opt/(exp(t_opt/2)+1+t_opt/2);
end

%plot(Nvec,aoi_oma, Nvec,Aoi_ana_oma, '-s',Nvec,aoi, Nvec, Aoi_ana,'-o') 
plot(Mvec,aoi_oma, Mvec,Aoi_ana_oma, '-s',Mvec,aoi, Mvec, Aoi_ana,'-o', Mvec, Aoi_app) 
%plot(Mvec, Aoi_ana./Aoi_ana_oma, Mvec, ratiox) 
%plot(Mvec, Aoi_ana./Aoi_ana_oma ) 
